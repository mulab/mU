...
inline mpf_ptr mpf_new(mpf_srcptr x) {
    mpf_ptr r = mpf_new();
    mpf_set(r, x);
    return r;
}
...
inline mpf_ptr mpf_clone(mpf_srcptr x) {
    mpf_ptr r = new __mpf_struct;
    *r = *x;
    return r;
}
inline mpf_ptr mpf_new(mpfr_srcptr x) {
    mpf_ptr r = mpf_new();
...
#undef DEF
#define DEF(name,f)\
var name(const Real& x, const Real& y)\
{\
	mpfr_ptr rr = mpfr_new();\
	mpfr_ptr xr = mpfr_new(x.mpf);\
