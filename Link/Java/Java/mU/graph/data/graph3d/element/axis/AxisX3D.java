...
public class AxisX3D extends Axis3D {

	private static final long serialVersionUID = -3982198640728313892L;
	private static final Point3d wordMove = new Point3d(-0.03,-0.05,-0.1);
	...
	protected Shape3D getLine(){
		...
		coods[0] = trans(new Point3d(start,height[1],height[2]));
		coods[1] = trans(new Point3d(end,height[1],height[2]));
	...
	protected Shape3D getRuler(){
		...
		for(int i=0; i<rulers.valuedruler.size(); i++){
			cood[2*i] = trans(new Point3d(rulers.valuedruler.get(i).getPos(),height[1],height[2]));
			cood[2*i+1] = trans(new Point3d(rulers.valuedruler.get(i).getPos(),height[1]+rullen,height[2]));
		}
	...
	public TextTransformGroup[] getStaticElement(){
		...
		for(int i=0; i<rulers.valuedruler.size(); i++){
			Point3d pos = trans(new Point3d(rulers.valuedruler.get(i).getPos(),height[1],height[2]));
