...
void PolynomialInitialize()
{
	...
	random::randomize();
	Begin(System);
	CProcs[Sym(L"Factor")] = WRAP(Factor);
	CProcs[Sym(L"FactorList")] = WRAP(FactorList);
	CProcs[Sym(L"CharacterSet")]= WRAP(CharacterSet);
	CProcs[Sym(L"Cyclotomic")]= WRAP(Cyclotomic);
	CProcs[Sym(L"CyclotomicQ")]= WRAP(CyclotomicQ);
	CProcs[Sym(L"test")]= WRAP(Test);
	CProcs[Sym(L"Decompose")]= WRAP(Decompose);
	CProcs[Sym(L"PolynomialGCD")]= WRAP(PolynomialGCD);
	CProcs[Sym(L"Solve")]= WRAP(Solve);
	CProcs[Sym(L"ZRoot")]= WRAP(ZRoot);
	CProcs[Sym(L"GroebnerBasis")]= WRAP(GroebnerBasis);
	CProcs[Sym(L"RootIntervals")]= WRAP(RootIntervals);
	End();
	UniSolveInitialize();
	ComplexInitialize();
}
